...
IMG?=gcr.io/spectro-dev-public/stylus/${ISO_NAME}:latest
...
qcow2-image:
	docker buildx build -t $(IMG) -f Image.Dockerfile --build-arg IMG_NAME=$(ISO_NAME).qcow2 --push .

vmdk-image:
	docker buildx build -t $(IMG) -f Image.Dockerfile --build-arg IMG_NAME=$(ISO_NAME).vmdk --push .
